...
if [ -f ${base}.nii ] && [ -f ${base}.nii.gz ]; then
  ...
  if [ -f ${outputdir}/${subj}/scripts/IsRunning.lh+rh ]; then
    ...
    while true; do
      ...
      && [ -f ${outputdir}/${subj}/mri/orig/001.mgz ] ; then
        ...
        echo "----------------------- "
        sleep 4
        (recon-all -subjid ${subj} -all -openmp ${NCORES} -subfields -cw256 ; \
        ...
        (recon-all -subjid ${subj} -i ${T1w_noneck} -all -openmp ${NCORES} -subfields -cw256 ; \
        /neurodocker/combine_subnuclei.sh ${outputdir} ${subj}) | tee ${outputdir}/${subj}_recon.log &
...
echo "continue to thalamus subsegmentation"
		...
		(recon-all -subjid ${subj} -subfields -openmp ${NCORES} -cw256 ; \
